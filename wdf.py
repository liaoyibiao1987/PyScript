...
def waitForLogin():
    ...
    if code == '201':  # 已扫描
        ...
        services = [('wx2.qq.com', 'webpush2.weixin.qq.com'),
...
def webwxinit():
    ...
    url = base_uri + \
        '/webwxinit?pass_ticket=%s&skey=%s&r=%s' % (pass_ticket, skey, int(time.time()))
    params = {
...
def webwxgetcontact():
    ...
    url = base_uri + \
        '/webwxgetcontact?pass_ticket=%s&skey=%s&r=%s' % (pass_ticket, skey, int(time.time()))
...
def createChatroom(UserNames):
    ...
    url = base_uri + \
        '/webwxcreatechatroom?pass_ticket=%s&r=%s' % (pass_ticket, int(time.time()))
    params = {
        'BaseRequest': BaseRequest,
...
def main():
    ...
    d = {}
    for Member in MemberList:
        d[Member['UserName']] = (Member['NickName'].encode('utf-8'), Member['RemarkName'].encode('utf-8'))
    print('开始查找...')
