...
#    with request.urlopen('http://api.map.baidu.com/telematics/v3/weather?location=' + cityName + '&output=json&ak=hgfHY5GP43WxRusYgVQYKTzFfV3DfUbY') as f:  
#        data = f.read()  
...
#北上广深
cityList_bsgs = [{'code':"101010100", 'name':"北京"},
...
def getCityWeatherDetail_SixDay(cityID):
    url = "http://m.weather.com.cn/data/" + str(cityID) + ".shtml"
    try:
        stdout = urllib.request.urlopen(url)
        weatherInfomation = stdout.read().decode('utf-8')
        jsonDatas = json.loads(weatherInfomation)
